...
class StereoCameraCalibrationPreviewFactory(PreviewControllerFactory):

    def create(self, workspace, name, widget_parent):
        return StereoCameraCalibrationPreview(parent=self.parent,
                                        context=self.context,
                                        widget_parent=widget_parent,
                                        widget_name=name,
                                        workspace=workspace)

class StereoCameraCalibrationPreview(PreviewControllerBase):
    ...
    def setupPreview(self):
        ...
        with enaml.imports():
            from .views.stereocamera_calibration import StereoCameraCalibrationPreviewContent
            from utinteractiveconsole.plugins.calibration.views.live_preview import LivePreview

        self.content = StereoCameraCalibrationPreviewContent(parent=self.widget_parent, controller=self)
        self.content.initialize()

        # create and show preview
        self.parent.preview = LivePreview(name="%s_preview" % self.widget_name,
                                          title="Stereo Camera Calibration Preview",
                                          controller=self,
